...
public class SmoothMouseLook : MonoBehaviour {
	...
	Vector3 TargetPoint;
	[HideInInspector]
	public Quaternion DinoQuaternion;
	...
	void Update(){
		...
		if(Time.timeScale > 0.0f && Time.smoothDeltaTime > 0.0f && !InputControl.instance.AutoAm)
		...
		if (InputControl.instance.AutoAm)
        {
			if (Animals)
			{
				...
				rotationX = 0f;
				rotationY = 0f;
				TargetPoint = Animals.transform.position - transform.position;
				DinoQuaternion = Quaternion.LookRotation(TargetPoint, Vector3.up);
				originalRotation.y = DinoQuaternion.y;
				originalRotation.w = DinoQuaternion.w;
				myTransform.rotation = Quaternion.Slerp(transform.rotation, DinoQuaternion, Time.fixedDeltaTime * LookingSpeed);
...
public static float ClampAngle (float angle, float min, float max){
	...
	public void Set_rotation(float playerrotation)
	{
		//if (rotateon == true)
		//{
		rotationX = playerrotation;
		rotateon = false;
		//}
	}
}