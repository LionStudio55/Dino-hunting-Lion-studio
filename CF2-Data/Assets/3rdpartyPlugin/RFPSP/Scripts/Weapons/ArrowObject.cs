		...
		&& !arrowRayHit.collider.gameObject.GetComponent<ArrowObject>()){
			...
			case 13://hit object is an NPC
				...
				if(hitCol.gameObject.GetComponent<LocationDamage>() && hitCol.gameObject.GetComponent<LocationDamage>().AIComponent.enabled){
					...
					if (hitCol.gameObject.GetComponent<Hit_Body>())
					{
						hitCol.gameObject.GetComponent<Hit_Body>().damageManage.ApplyDamage((int)(damage + damageAddAmt), transform.forward, Camera.main.transform.position.x, 0);
						//hit.collider.gameObject.GetComponent<Hit_Head>().RootObject.GetComponent<DamageManager>().hp = 0;
						UI_Manager.shot_org = true;
					}
					if (hitCol.gameObject.GetComponent<Hit_Head>())
					{
						hitCol.gameObject.GetComponent<Hit_Head>().damageManage.ApplyDamage(hitCol.gameObject.GetComponent<Hit_Head>().damageManage.hp, transform.forward, Camera.main.transform.position.x, 0);
						//hit.collider.gameObject.GetComponent<Hit_Head>().damageManage.hp = 0;
						UI_Manager.shot_org = false;

					}
					////Emerald AI Damage
					//if (hitCol.gameObject.GetComponent<EmeraldAI.EmeraldAISystem>() != null)
					//{
					//	hitCol.gameObject.GetComponent<EmeraldAI.EmeraldAISystem>().Damage((int)(damage + damageAddAmt), EmeraldAI.EmeraldAISystem.TargetType.Player);
					//	FPSPlayerComponent.UpdateHitTime();//used for hitmarker
					//}
