...
//        Debug.Log("gun script time value :"+Time.timeScale);
	...
	public void sliderValue()
    {
		...
		UI_Manager.instance.ZoomText.text = (mainSlider.value * 3).ToString("0")+"X";
        //print("Slider Value "+mainSlider.value);
        //print("Slider Value ZooM " + NormalCamera.fieldOfView);
    }
	...
	public void  Zoom ()
	{
        ...
        if(Zooming)
        {
            ...
            crossHairOwnUse.gameObject.SetActive(true);
            simpleAim.gameObject.SetActive(false);
			if (Zoom_in && audiosource != null)
			{
				audiosource.PlayOneShot(Zoom_in);
			}
		}
        else
        {
				...
				if (AmmoIn > 0) {
					...
					for (int i=0; i<BulletNum; i++) {
						if (Bullets) {
							...
							Debug.Log("camfollow" + UI_Manager.onFollowCam);
                            if(simpleAim.color == Color.green && DamageManager.KilledAnimal == LevelsHandler.instance.Total_TargetCount - 1 && camRAy.instancee.distace_con>20f&&UI_Manager.onFollowCam)// && camRAy.instancee.slowmo_health == true)//Inam
                            {
    ...
    IEnumerator GunParts()
    {
      ...
      // followCam.transform.DOLookAt(bullet.transform.position,0.01f);
       ...
       //followCam.transform.DOMove(bullet.transform.Find("point").transform.position, 0.005f);
        ...
        followCam.GetComponent<CameraFilterPack_Blur_Radial_Fast>().enabled = true; // blur effect on 
        followCam.GetComponent<AudioListener>().enabled = true; // blur effect on 
        followCam.GetComponent<AudioSource>().enabled = true; // Sound effect on 
