...
namespace PedestrianSystem{
	...
	public class PedestrianSystemManager : MonoBehaviour {
        ...
        {
			if (DamageManager.KilledAnimal % /*2*/ Random.Range(2,3) == 0)
			{
				try
				{
				...
				UI_Manager.instance.AdloadingBreak.SetActive(true);
				Invoke(nameof(AdloadingOn),2f);
			}
		}
		private void AdloadingOn()
		{
			UI_Manager.instance.AdloadingBreak.SetActive(true);
			try
			{
			...
			CancelInvoke(nameof(AdloadingOn));
			Invoke(nameof(AdloadingOff), 1f);
		}

		private void AdloadingOff()
		{
			UI_Manager.instance.AdloadingBreak.SetActive(false);
			try
			{
				if (FindObjectOfType<MediationHandler>())
					FindObjectOfType<MediationHandler>().LoadInterstitial();
			...
			CancelInvoke(nameof(AdloadingOff));
		}
