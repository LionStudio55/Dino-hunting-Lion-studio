...
public class AdmobAdsManager : MediationHandler
{
    ...
    {
        if (!isAdmobInitialized || IsVideoAdReady() || vAdStatus == AdLoadingStatus.Loading || Constantss.adsRemoteConfigStatus == "0" || adsStatus == AdType.NOAds || Noads())
        {
            return;
        }
        if (Application.internetReachability == NetworkReachability.ReachableViaCarrierDataNetwork | Application.internetReachability == NetworkReachability.ReachableViaLocalAreaNetwork)
    ...
    {
        if (!isAdmobInitialized || IsRewardedAdReady() || rAdStatus == AdLoadingStatus.Loading || Constantss.adsRemoteConfigStatus == "0" || Noads())
        {
            return;
        }
    ...
    {
        if (!isAdmobInitialized || IsRewardedInterstitialAdReady() || riAdStatus == AdLoadingStatus.Loading || Constantss.adsRemoteConfigStatus == "0" || Noads())
        {
