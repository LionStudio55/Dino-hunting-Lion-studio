...
public class WeaponBehavior : MonoBehaviour
{
    ...
    public bool scropeimage;
    public RaycastHit templast;
    public Vector3 tempdirection;
    public GameObject currentbullet;
    public bool time1 = false, time2 = false;

    [Header("Slomotion bullet")]
    public bool Gunhasslomo = false;
    public GameObject slomobullet;
    public GameObject Muzzleeffect;
    private bool slomostart = false;
    ...
    void LateUpdate()
    {
        ...
        {//allow pausing by setting timescale to 0
            ...
            if (FPSPlayerComponent.zoomed && !zoomIsBlock)
            {
                ...
                if (scropeimage == true)
                {
                    mainCamTransform.GetComponent<CameraControl>().Scope_image.SetActive(true);
                  //  mainCamTransform.GetComponent<CameraControl>().scopegunmesh.SetActive(false);

                    //mainCamTransform.GetComponent<CameraControl>().zoomslider.SetActive(true);

                    //UI_Manager.instance.zoom_slider.gameObject.SetActive(true);
            ...
            else
            {
                if (scropeimage == true)
                {
                    mainCamTransform.GetComponent<CameraControl>().Scope_image.SetActive(false);
                   // mainCamTransform.GetComponent<CameraControl>().scopegunmesh.SetActive(true);

                    //mainCamTransform.GetComponent<CameraControl>().zoomslider.SetActive(false);

                    //UI_Manager.instance.zoom_slider.gameObject.SetActive(false);
    ...
    void HitObject(RaycastHit hit, Vector3 directionArg, bool isSecondCast = false)
    {
        ...
        switch (hit.collider.gameObject.layer)
        {
            ...
            case 1://hit object is an object with transparent effects like a window
                ...
                break;
            case 13://hit object's layer is an NPC

                if (Constants.Getprefs(Constants.lastselectedMode) == 2)
                {
                    if (hit.collider.gameObject.GetComponent<Hit_Head>())
                    {
                        hit.collider.gameObject.GetComponent<Hit_Head>().damageManage.ApplyDamage(hit.collider.gameObject.GetComponent<Hit_Head>().damageManage.hp, directionArg, mainCamTransform.position.x, 0);
                        UI_Manager.shot_org = false;
                    }
                    else if (hit.collider.gameObject.GetComponent<Hit_Body>())
                    {
                        hit.collider.gameObject.GetComponent<Hit_Body>().damageManage.ApplyDamage((int)damageAmt, directionArg, mainCamTransform.position.x, 0);
                ...
                else
                {
                    ...
                    if (hit.collider.gameObject.GetComponent<Hit_Body>() && MissionHandler.ins.Total_TargetCount - DamageManager.KilledAnimal != 1 && Constants.Getprefs(Constants.lastselectedMode) != 2)
                    {
                        hit.collider.gameObject.GetComponent<Hit_Body>().damageManage.ApplyDamage((int)damageAmt, directionArg, mainCamTransform.position.x, 0);
                        UI_Manager.shot_org = true;
                    }
                    else if (hit.collider.gameObject.GetComponent<Hit_Head>() && MissionHandler.ins.Total_TargetCount - DamageManager.KilledAnimal != 1 && Constants.Getprefs(Constants.lastselectedMode) != 2)
                    {
                        hit.collider.gameObject.GetComponent<Hit_Head>().damageManage.ApplyDamage(hit.collider.gameObject.GetComponent<Hit_Head>().damageManage.hp, directionArg, mainCamTransform.position.x, 0);
                        UI_Manager.shot_org = false;
                    }

                    else if (hit.collider.gameObject.GetComponent<Hit_Body>() && MissionHandler.ins.Total_TargetCount - DamageManager.KilledAnimal == 1 && Constants.Getprefs(Constants.lastselectedMode) != 2)
                    {
                        if (Gunhasslomo)
                        {
                            canShoot = false;
                            ...
                            if (slomobullet.GetComponent<BulletHandler>())
                                slomobullet.GetComponent<BulletHandler>().target = hit.collider.transform.position;
                            if (hit.collider.transform.root.gameObject.GetComponent<Emerald_Animal_AI>())
                                hit.collider.transform.root.gameObject.GetComponent<Emerald_Animal_AI>().Stop();
                            else if (hit.collider.transform.root.gameObject.GetComponent<EmeraldAI.EmeraldAISystem>())
                                hit.collider.transform.root.gameObject.GetComponent<EmeraldAI.EmeraldAISystem>().Stop();
                            else if (hit.collider.gameObject.GetComponent<Hit_Body>().damageManage.gameObject.GetComponent<SWS.splineMove>())
                            {
                                // print("stopsplinemove");
                                hit.collider.gameObject.GetComponent<Hit_Body>().damageManage.gameObject.GetComponent<SWS.splineMove>().Stop();
                            }
                            //else if (hit.collider.gameObject.GetComponent<Hit_Head>().damageManage.gameObject.GetComponent<SWS.splineMove>())
                            //{
                            //    // print("stopsplinemove");
                            //    hit.collider.gameObject.GetComponent<Hit_Head>().damageManage.gameObject.GetComponent<SWS.splineMove>().Stop();

                            //}
                            if (slomobullet)
                            {
                                slomobullet.transform.SetParent(null);
                                slomobullet.SetActive(true);
                            }
                        }
                        else
                        {
                            if (hit.collider.gameObject.GetComponent<Hit_Body>())
                            {
                                hit.collider.gameObject.GetComponent<Hit_Body>().damageManage.ApplyDamage((int)damageAmt, directionArg, mainCamTransform.position.x, 0);
                                UI_Manager.shot_org = true;
                            }
                        ...
                        if (UI_Manager.instance)
                            UI_Manager.instance.controls_status();
                    }
                    else if (hit.collider.gameObject.GetComponent<Hit_Head>() && MissionHandler.ins.Total_TargetCount - DamageManager.KilledAnimal == 1 && Constants.Getprefs(Constants.lastselectedMode) != 2)
                    {
                        if (Gunhasslomo)
                        {
                            ...
                            //    print("stopsplinemove");
                            //    hit.collider.gameObject.GetComponent<Hit_Body>().damageManage.gameObject.GetComponent<SWS.splineMove>().Stop();
                            //    hit.collider.gameObject.GetComponent<Hit_Head>().damageManage.gameObject.GetComponent<Animator>().enabled = false;
                            //}
                            else if (hit.collider.gameObject.GetComponent<Hit_Head>().damageManage.gameObject.GetComponent<SWS.splineMove>())
                            {
                                print("stopsplinemove");
                        ...
                        else
                        {
                            if (hit.collider.gameObject.GetComponent<Hit_Head>())
                            {
                                hit.collider.gameObject.GetComponent<Hit_Head>().damageManage.ApplyDamage((int)damageAmt, directionArg, mainCamTransform.position.x, 0);
                                UI_Manager.shot_org = false;
                            }
                        }
                        if (UI_Manager.instance)
                ...
                //print("Last kill :" +(MissionHandler.ins.Total_TargetCount - DamageManager.KilledAnimal));
                ...
                //    {
                //        hit.collider.gameObject.GetComponent<Hit_Body>().damageManage.ApplyDamage((int)damageAmt, directionArg, mainCamTransform.position.x, 0);
                //        //hit.collider.gameObject.GetComponent<Hit_Head>().RootObject.GetComponent<DamageManager>().hp = 0;
                //        UI_Manager.shot_org = true;
                //    }
                //    if (hit.collider.gameObject.GetComponent<Hit_Head>())
                //    {
                //        hit.collider.gameObject.GetComponent<Hit_Head>().damageManage.ApplyDamage(hit.collider.gameObject.GetComponent<Hit_Head>().damageManage.hp, directionArg, mainCamTransform.position.x, 0);
                //        //hit.collider.gameObject.GetComponent<Hit_Head>().damageManage.hp = 0;
                //        UI_Manager.shot_org = false;
                //    }
                //}
                break;
